/// @description Full Render: Juggernaut Boss + Semua Upgrade
...
if (!boss_fully_dead) {
    ...
    // ===================================================================
    // LAYER 3: KOTAK TURRET + CRACK VISUAL
    // ===================================================================
    ...
    // CRACK KIRI (makin banyak sesuai damage)
    if (phase == 2 && !bottom_destroyed && core_bottom_hp < 5) {
        ...
        for (var ci = 0; ci < crack_count; ci++) {
            var cx1 = lx + random_range(-50, 50);
            var cy1 = target_l_y + random_range(-40, 40);
    ...
    // --- KOTAK KANAN --- (vertex fix simetris)
    ...
    if (phase == 2 && !top_destroyed && core_top_hp < 5) {
        ...
        for (var ci = 0; ci < crack_count_r; ci++) {
            var crx1 = rx + random_range(-50, 50);
            var cry1 = target_r_y + random_range(-40, 40);
    ...
    // ===================================================================
    // LEDAKAN BERANTAI (FRAME 600-450)
    // ===================================================================
    ...
    // ===================================================================
    // LAYER 4: PARTIKEL PUING MELAYANG
    // ===================================================================
    if (phase == 3 && magnet_progress < 1.0) {
        var seed_kid = 54321; random_set_seed(seed_kid);
        var ground_level = 710;
        for (var i = 0; i < 35; i++) {
            var start_x_l    = lx + random_range(-55, 55);
            var start_x_r    = rx + random_range(-55, 55);
    ...
    // ===================================================================
    if (phase == 3 && magnet_progress >= 1.0) {
        var tether_color = is_enraged ? make_color_rgb(255, 80, 0) : make_color_rgb(0, 240, 255);
        draw_set_alpha(random_range(0.6, 1.0));
        draw_set_color(tether_color); draw_line_width(lx, laser_tether_y, rx, laser_tether_y, 10);
        draw_set_color(c_white);      draw_line_width(lx, laser_tether_y, rx, laser_tether_y, 3);
        draw_set_color(tether_color); draw_line_width(lx, laras_y_offset, lx, laser_tether_y, 4); draw_line_width(rx, laras_y_offset, rx, laser_tether_y, 4);
        draw_set_alpha(1.0);
    }

    // LAYER 6: LARAS MERIAM
    ...
    if (!bottom_destroyed && phase != 3 && jeda_fase3 <= 0) {
        ...
        draw_set_color(c_armor_mid); draw_line_width(lx, laras_y_offset, tl_x, tl_y, 16);
    }
    draw_set_color(c_border);
    if (!top_destroyed && phase != 3 && jeda_fase3 <= 0) {
        ...
        draw_set_color(c_armor_mid); draw_line_width(rx, laras_y_offset, tr_x, tr_y, 16);
    }
    ...
    // ===================================================================
    // SURVIVAL TIMER VISUAL
    // ===================================================================
    if (phase == 3 && fase3_mode != 4 && survival_timer > 0) {
        var max_surv    = 30 * room_speed;
        var surv_ratio  = survival_timer / max_surv;
        var surv_radius = 22;
        var surv_x      = room_width - 55;
        var surv_y      = 55;
        ...
        for (var seg = 0; seg < segments; seg++) {
            var seg_ratio = seg / segments;
            if (seg_ratio > (1 - surv_ratio)) {
                var a1  = -90 + (seg_ratio * 360);
                var a2  = -90 + ((seg + 1) / segments * 360);
                var sx1 = surv_x + lengthdir_x(surv_radius, a1);
                var sy1 = surv_y + lengthdir_y(surv_radius, a1);
                var sx2 = surv_x + lengthdir_x(surv_radius, a2);
                var sy2 = surv_y + lengthdir_y(surv_radius, a2);
                draw_line_width(sx1, sy1, sx2, sy2, 6);
            }
        }
        ...
        draw_set_font(fnt_boss_name);
        draw_text(surv_x, surv_y, "4");
        draw_set_halign(fa_left); draw_set_valign(fa_top);

        if (surv_ratio < 0.2) {
            var blink = (current_time div 150) mod 2 == 0;
            if (blink) {
    ...
    if (warning_active && warning_timer > 0) {
        ...
        draw_set_color(make_color_rgb(255, 60, 60));
        draw_line_width(cx - 18, cy - 18, cx + 18, cy + 18, 5);
        draw_line_width(cx + 18, cy - 18, cx - 18, cy + 18, 5);
        draw_set_color(c_white);
        draw_line_width(cx - 18, cy - 18, cx + 18, cy + 18, 2);
        draw_line_width(cx + 18, cy - 18, cx - 18, cy + 18, 2);
        draw_set_halign(fa_center); draw_set_valign(fa_middle);
        draw_set_font(fnt_boss_name);
        draw_text(cx, cy + 42, "!!");
    ...
    // ===================================================================
    // CROSS-FIRE WARNING (marker titik tengah pemain)
    // ===================================================================
